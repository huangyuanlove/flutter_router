...
echo "扫描 app/build.gradle 中 minSdkVersion "
if [ `grep -c 'minSdkVersion 19' .android/app/build.gradle` -eq '1' ]; then
     echo "minSdkVersion 已经是19 ，不需要修改"
else
     echo "minSdkVersion 需要修改为 19 "
     sed -i '' 's/minSdkVersion 16/minSdkVersion 19/g' .android/app/build.gradle
fi
...
echo "扫描 Flutter/build.gradle 中 minSdkVersion "
if [ `grep -c 'minSdkVersion 19' .android/Flutter/build.gradle` -eq '1' ]; then
     echo "minSdkVersion 已经是19 ，不需要修改"
else
     echo "minSdkVersion 需要修改为 19 "
     sed -i '' 's/minSdkVersion 16/minSdkVersion 19/g' .android/Flutter/build.gradle
fi
...
gradle clean flutter:assembleRelease --info

if [ $? -eq 0 ]; then
    echo ''
else
    echo 'assembleRelease 出错 !!!'
    delFatAarConfig
    exit 1
fi
...
cd ${projectDir}/.android/Flutter/

mv build/outputs/aar/flutter-release.aar build/outputs/aar/flutter-release_$flutterAArVersion.aar
